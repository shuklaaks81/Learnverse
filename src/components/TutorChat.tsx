...
export function TutorChat() {
  ...
  const handleClearHistory = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Don't clear the UI messages, just clear the tutor's memory
        sounds.current?.playCorrect();
        setError('');
        // Show a success message
        alert('🧹 Tutor memory cleared! The tutor will start fresh on your next question!');
      }
    } catch (err) {
    ...
    <div className="w-full max-w-2xl mx-auto bg-gradient-to-br from-purple-100 via-blue-50 to-pink-100 rounded-3xl p-6 shadow-2xl border-2 border-purple-200 backdrop-blur-sm">
      ...
      <div className="flex justify-between items-center text-xs font-semibold text-gray-600 pt-4 border-t border-purple-200">
        <div className="flex gap-4">
          <span>📊 Questions: <span className="text-purple-600 font-bold">{messages.filter(m => m.type === 'question').length}</span>/50</span>
          <span>💬 Answers: <span className="text-blue-600 font-bold">{messages.filter(m => m.type === 'answer').length}</span></span>
        </div>
        <button
          onClick={handleClearHistory}
          className="px-4 py-2 bg-gradient-to-r from-orange-400 to-red-400 hover:from-orange-500 hover:to-red-500 text-white rounded-full font-bold text-xs shadow-md hover:shadow-lg transition-all transform hover:scale-105 active:scale-95"
          title="Clear tutor's memory (starts fresh conversation)"
        >
          🧹 Clear Memory
        </button>
      </div>
