...
export default function BloxdScriptMaker() {
  ...
  const reportError = async () => {
    ...
    setIsGenerating(true);
    setChatMessages(prev => [...prev, { role: "user", message: `Error Report: ${errorReport}` }]);
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: "system", message: "✅ Fixed the error! Check the updated code." }]);
      setConversationHistory(prev => prev + `\nError reported: ${errorReport}\nFixed: Success\n`);
      setErrorReport("");
    } catch (err) {
      console.error(err);
      setThinkingStep("");
      setChatMessages(prev => [...prev, { role: "system", message: "❌ Couldn't fix the error automatically. Try describing it differently!" }]);
    } finally {
      setIsGenerating(false);
    }
  };
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-4 justify-center">
              <button
                ...
                className={`flex-1 py-3 px-6 rounded-xl font-bold transition transform hover:scale-105 ${
                  scriptMode === "world"
              ...
              >
                🌍 WORLD CODE MODE
                <div className="text-xs mt-1 opacity-80">Full world scripts with player interaction</div>
              </button>
              <button
              ...
              >
                📦 CODE BLOCK MODE
                <div className="text-xs mt-1 opacity-80">Compact scripts for code blocks</div>
              </button>
            </div>
          </div>
